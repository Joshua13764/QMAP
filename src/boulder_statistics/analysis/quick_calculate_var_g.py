...
class FittingFunction():
    ...
    def F(self, alphas, e, g_a, g_b, q) -> np.ndarray:
        ...
        total_p_alpha_sample = (alphas_to_sample ** (- q - 1)) * sigma_sum(
            ...
            phis=0.5 *
            (Phi_counts_smoothed_bins[1:] + Phi_counts_smoothed_bins[:-1]),
    ...
    def cleaned_data(self) -> LazyFrame:
        target = boulder_agg_data.filter(
            pl.col("longest_axis_diameter") *
            1000 > self.LAD_min,
            pl.col("alpha") < 1e3).collect()
        most = boulder_agg_data.filter(
            pl.col("longest_axis_diameter") *
            1000 > self.LAD_min).collect()

        most_ratios = most["longest_axis_diameter"].to_numpy(
        ) / most["surface_area"].to_numpy()
        target_ratios = target["longest_axis_diameter"].to_numpy(
        ) / target["surface_area"].to_numpy()
