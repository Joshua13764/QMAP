...
from bennu_feature_extractor.environment_tools.base_classes.fs_medium_base import \
    FileStorageMediumBase
...
class LocalDisk(FileStorageMediumBase):
    ...
    def add_local_disk_file(self, virtual_path: Path) -> Path:
        if self.does_path_exist(virtual_path):
            raise FileExistsError(
                f"The virtual file path {virtual_path} all-ready exits in LocalDisk")
    ...
    def get_file_path_to_write(self, virtual_path: Path) -> Path:
        ...
        file_path: Path = self.local_disk_files[virtual_path.as_uri()].get_actual_path(
            self.root_path) if self.does_path_exist(virtual_path)else self.add_local_disk_file(virtual_path)
