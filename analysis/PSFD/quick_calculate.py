...
class FittingFunction():
    ...
    def sigma_sum(self, alphas, phis, phi_weights, q) -> np.ndarray:
        return np.sum(np.where(
            (self.a_min < alphas[:, None] / phis[None, :]) & (alphas[:, None] / phis[None, :] < self.a_max),
            phi_weights[None, :] * (- q * phis[None, :] ** (q)) / (self.a_max ** (-q) - self.a_min  ** (-q)),
            0
    ...
    def F(self, alphas, effectiveness, g, q) -> np.ndarray:
        ...
        total_p_alpha = (alphas ** (- q - 1)) * self.sigma_sum(
            alphas = alphas,
            phis = 0.1 * g * 0.5 * (Phi_counts_smoothed_bins[1:] + Phi_counts_smoothed_bins[:-1]),
